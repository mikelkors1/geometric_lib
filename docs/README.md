...
## <span style="color: #0000FF;">Тестирование</span>

Библиотека включает набор юнит-тестов для проверки работоспособности всех функций.

### Запуск тестов:

```bash
python3 -m unittest tests.test1 -v
```

### Описание тестов:

- **Файл с тестами**: `tests/test1.py`
- **Количество тестов**: 36+ тестовых случаев
- **Покрытие**: 100% функций библиотеки (8 функций в 4 модулях)

### Что проверяют тесты:

- Корректность вычисления площадей для всех фигур
- Корректность вычисления периметров для всех фигур
- Обработку различных типов входных данных (положительные числа, нули, дробные числа, большие значения)
- Граничные случаи (нулевые значения, единичные значения)

Все тесты содержат подробные комментарии на русском языке, объясняющие, что проверяет каждый тест.
